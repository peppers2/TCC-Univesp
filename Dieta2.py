...
opcoes_doencas = ['Diabetes', 'Hipertensão',
                  'Colesterol Alto', 'Obesidade', 'Nenhuma']
...
def filtrar_alimentos_por_condicao(condicao, alimentos):
    ...
    alimentos_filtrados = {
        k: v for k, v in alimentos.items() if k not in restricoes[condicao]}
...
def sugerir_refeicoes(alimentos, calorias_por_refeicao):
    ...
    for i in range(4):
        # Embaralha a lista a cada nova refeição para maior variedade
...
try:
    ...
    st.error(
        "Modelo de Machine Learning não encontrado. Por favor, treine e forneça o modelo adequado.")
    st.stop()
...
opcao = st.sidebar.radio('Escolha uma opção', [
                         'Calculadora de Gasto Calórico', 'Sugestão de Dieta com Machine Learning', 'Montar Dieta Manualmente'])
st.sidebar.write("")
...
if opcao == 'Calculadora de Gasto Calórico':
    ...
    st.write(
        f"Seu gasto calórico diário, considerando o nível de atividade física, é: {gasto_calorico:.2f} calorias por dia.")
    ...
    alimentos_filtrados = filtrar_alimentos_por_condicao(
        condicao_saude, alimentos_calorias)
    refeicoes, calorias_refeicoes = sugerir_refeicoes(
        alimentos_filtrados, calorias_por_refeicao)
        ...
        if refeicao:
            ...
            for j, alimento in enumerate(refeicao):
                with colunas[j]:
                    ...
                    st.write(
                        f"{alimento} - Calorias: {alimentos_calorias[alimento]['calorias']}")
...
if opcao == 'Montar Dieta Manualmente':
    st.title('Monte sua dieta manualmente')
    ...
    calorias_por_refeicao = (
        tmb * fatores_atividade[nivel_atividade]) / 4

    st.write(
        f"Serão adicionadas aproximadamente {calorias_por_refeicao:.2f} Calorias por refeição")

    st.subheader('Escolha os tipos dos alimentos que você consome:')
    tipos_alimentos = st.multiselect(
        'Tipos de alimentos', list(alimentos.tipos))

    st.write(tipos_alimentos)

    st.subheader('Escolha os alimentos do café da manhã:')
    st.write('Selecione os alimentos que você deseja adicionar ao seu café da manhã:')
    tipos_alimentos_cafe_manha = st.multiselect(
        'Tipos de alimentos', list(tipos_alimentos), key='cafe_manha')

    alimentos_possiveis = alimentos.todos_alimentos[
        alimentos.todos_alimentos['Tipo'].isin(tipos_alimentos)]

    st.subheader('Alimentos sugeridos para o café da manhã:')
    st.write('Abaixo uma lista de alimentos sugeridos para café da manhã de acordo com o limite de calorias disponíveis por refeição:')

    alimentos_possiveis_cafe_manha = alimentos_possiveis[
        alimentos_possiveis['Tipo'].isin(tipos_alimentos_cafe_manha)]

    calorias_cafe_manha = 0

    st.write(calorias_por_refeicao)

    st.markdown('### Alimentos possíveis para o café da manhã:')
    st.write(alimentos_possiveis_cafe_manha)

    st.subheader('Escolha os alimentos do almoço:')
    st.write('Selecione os alimentos que você deseja adicionar ao seu almoço:')
    tipos_alimentos_almoco = st.multiselect(
        'Tipos de alimentos', list(tipos_alimentos), key='almoco')

    alimentos_possiveis_almoco = alimentos_possiveis[
        alimentos_possiveis['Tipo'].isin(tipos_alimentos_almoco)]

    st.markdown('### Alimentos possíveis para o almoço:')
    st.write(alimentos_possiveis_almoco)

    st.subheader('Escolha os alimentos do lanche da tarde:')
    st.write(
        'Selecione os alimentos que você deseja adicionar ao seu lanche da tarde:')

    tipos_alimentos_lanche_tarde = st.multiselect(
        'Tipos de alimentos', list(tipos_alimentos), key='lanche_tarde')

    alimentos_possiveis_lanche_tarde = alimentos_possiveis[
        alimentos_possiveis['Tipo'].isin(tipos_alimentos_lanche_tarde)]
    st.markdown('### Alimentos possíveis para o lanche da tarde:')
    st.write(alimentos_possiveis_lanche_tarde)

    st.subheader('Escolha os alimentos do jantar:')
    st.write('Selecione os alimentos que você deseja adicionar ao seu jantar:')

    tipos_alimentos_jantar = st.multiselect(
        'Tipos de alimentos', list(tipos_alimentos), key='jantar')

    alimentos_possiveis_jantar = alimentos_possiveis[
        alimentos_possiveis['Tipo'].isin(tipos_alimentos_jantar)]
    st.markdown('### Alimentos possíveis para o jantar:')
    st.write(alimentos_possiveis_jantar)
