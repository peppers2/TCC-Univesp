...
if authentication_status:
   ...
   # authenticator.logout('Logout', 'sidebar')
    ...
    def sugerir_refeicoes(calorias_maximas, num_refeicoes=4):
        ...
        for alimento, info in alimentos:
            for i in range(num_refeicoes):
                if calorias_refeicoes[i] + info['calorias'] <= calorias_maximas / num_refeicoes:
    ...
    opcao = st.sidebar.radio('Escolha uma opção', ['Calculadora de Gasto Calórico', 'Seleção de Alimentos', 'Sugestão de Dieta', 'Sugestão de Refeições Aleatórias'])
    ...
    elif opcao == 'Sugestão de Refeições Aleatórias':
