...
alimentos_calorias = {
    ...
    'Banana': {'calorias': 105, 'imagem': 'https://upload.wikimedia.org/wikipedia/commons/thumb/8/8a/Banana_hemisphere.jpg/1200px-Banana_hemisphere.jpg'},
...
if opcao == 'Calculadora de Gasto Calórico':
    ...
    if alimento:
        ...
        st.write(f'{alimento}: {info["calorias"]} calorias por 100g')

elif opcao == 'Sugestão de Dieta':
    st.title('Sugestão de Dieta')
    
    calorias_max = st.number_input('Calorias Máximas por Refeição', min_value=0, value=500)
    
    refeicoes, calorias_refeicoes = sugerir_refeicoes(calorias_max)
    
    for i, refeicao in enumerate(refeicoes):
        st.subheader(f'Refeição {i + 1} - {calorias_refeicoes[i]:.2f} calorias')
    ...
    for i, refeicao in enumerate(refeicoes):
        calorias = calorias_refeicoes[i]
        fig.add_trace(go.Pie(labels=[alimento for alimento, _, _ in refeicao],
                             values=[calorias for _, calorias, _ in refeicao],
                             name=f'Refeição {i + 1}',
                             hole=0.4))
